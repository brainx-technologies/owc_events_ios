...
public class ReadMoreTextView: UITextView {
    ...
    func setupDefaults() {
        let defaultReadMoreText = NSLocalizedString("ReadMoreTextView.readMore", value: LocalizedKey.seeMore.string, comment: "")
        ...
        let attributedDefaultReadMoreText = NSAttributedString(string: defaultReadMoreText, attributes: [
            NSAttributedString.Key.foregroundColor: Color.seeMoreColor,
    ...
    private func pointIsInReadMoreOrReadLessTextRange(point _: CGPoint) -> Bool? {
        if needsTrim() {
            return false
        } else {
            return true
