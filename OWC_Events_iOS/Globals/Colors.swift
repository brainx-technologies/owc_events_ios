//
//  Colors.swift
//  ZyTrack
//
//  Created by BrainX Technologies on 2/19/21.
//

import Foundation
import UIKit

enum Color {
    static let backgroundDisabled = UIColor(0x929498)
    static let backgroundPrimary = UIColor(0xF4F4F7)
    static let black = UIColor(0x000000)
    static let borderGrey = UIColor(0x476992).withAlphaComponent(0.3)
    static let shadowColor = UIColor(0x25265E).withAlphaComponent(0.1)
    static let lightGrey = UIColor(0xD4DAE4)
    static let mainColor = UIColor(0x1D354A)
    static let selectedDisabledColor = UIColor(0xA2ACBA)
    static let textDarkGrey = UIColor(0x29304D)
    static let textGrey = UIColor(0x666666)
    static let tintColor = UIColor(0x385D89)
    static let splashColor = UIColor(0x013E6E)
    static let white = UIColor(0xFFFFFF)
    static let dateContainerColor = UIColor(0x7540EE)
    static let seeMoreColor = UIColor(0xFFC800)
    static let grayColor = UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.8)
    static let filterNumberBackground = UIColor(0xFF7052)
    static let monday = UIColor(0x2F61D5)
    static let tuesday = UIColor(0x22D369)
    static let wednesday = UIColor(0xFF7052)
    static let thursday = UIColor(0x787993)
    static let friday = UIColor(0x7540EE)
    static let saturday = UIColor(0x013E6E)
    static let sunday = UIColor(0x3FB6DC)
}
